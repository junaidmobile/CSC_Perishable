...
function ULDScanNoSlice() {

    let scanULDNo = $("#txtULDNoScan").val();

    if ($("#txtULDNoScan").val() == '') {
        return;
    }

    if (scanULDNo.length == '10') {

        const txtULDType = scanULDNo.slice(0, 3);
        const txtULDNumber = scanULDNo.slice(3, 8);
        const txtOwner = scanULDNo.slice(8, 10);

        $("#txtULDType").val(txtULDType);
        $("#txtULDNumber").val(txtULDNumber);
        $("#txtOwner").val(txtOwner);

    } else if (scanULDNo.length == '9') {
        const txtULDType = scanULDNo.slice(0, 3);
        const txtULDNumber = scanULDNo.slice(3, 7);
        const txtOwner = scanULDNo.slice(7, 9);

        $("#txtULDType").val(txtULDType);
        $("#txtULDNumber").val(txtULDNumber);
        $("#txtOwner").val(txtOwner);
    } else {
        errmsg = "Please scan valid ULD No.";
        $.alert(errmsg);
        $("#txtULDNoScan").val('');
        $("#txtULDType").val('');
        $("#txtULDNumber").val('');
