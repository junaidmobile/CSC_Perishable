...
function SaveTDGDetails() {
    ...
    if (errmsg == "" && connectionStatus == "online") {
        $.ajax({
            ...
            data: JSON.stringify({
                'pi_intALRowId': AWBid, 'pi_intTLRowId': 0,
                'pi_decScannedGrWt': txtReceivedPkgs, 'pi_decScannedGrWt': GrossWt,
                'pi_decVolumetricWt': volWeight, 'pi_strCreatedBy': window.localStorage.getItem("UserName"),
                'pi_strRemarks': '', 'pi_dtTDGDate': '',
                'pi_strTemperature': '', 'po_strOutput': ''
            }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            beforeSend: function doStuff() {
